...
def load_data(filename, svort, spress, svent, year_min=1960, year_max=2000):
...
def diff_bar_chart(vmax1, pmin1, vomax1, vmax2, pmin2, vomax2, labela):
    """
    Trace la différence de distribution (Dataset1 - Dataset2).
    Les arguments sont des listes (issues de load_data).
    """

    configs = [
        (vmax1, vmax2, "Vmax", (0, 100), 'skyblue'),
        (pmin1, pmin2, "Pmin", (800, 1020), 'salmon'),
        (vomax1, vomax2, "Vomax", (0, 500), 'lightgreen')
    ]

    fig, axs = plt.subplots(1, 3, figsize=(18, 5))

    for i, (d1, d2, title, x_range, color) in enumerate(configs):

        # Vérification données
        if len(d1) == 0 or len(d2) == 0:
            axs[i].set_title(f"Pas de données pour {title}")
            continue

        # Histogrammes
        counts1, edges = np.histogram(d1, bins=30, range=x_range)
        counts2, _ = np.histogram(d2, bins=30, range=x_range)

        # Normalisation en %
        pct1 = (counts1 / len(d1)) * 100
        pct2 = (counts2 / len(d2)) * 100
        ...
        axs[i].bar(bin_centers, diff, width=width, color=color, edgecolor='black')
        axs[i].axhline(0, color='black', linewidth=1.2)

        axs[i].set_title(f"$\\Delta$ {title}")
        axs[i].set_ylabel("Différence de densité (%)")
        axs[i].set_xlabel(f"N1:{len(d1)} | N2:{len(d2)}", fontsize=9)

    fig.suptitle(labela, fontsize=16, y = 0.02)

    #plt.tight_layout()
    #plt.subplots_adjust(top=0.5, bottom=0.11)
    plt.show()
...
def plot_relation(pmin_list, vmax_list, filename, ax=None, color='teal'):
    ...
    ax.plot(v_pred, pmin_smooth, color=color, linewidth=2,
             label=f"Simulation {filename}: \n $V_{{max}} = {alpha:.1f} \cdot \Delta P^{{{beta:.2f}}}$")
...
def get_cyclone_category_climatology(file_path, year_min, year_max, target_cat, ALADIN=False):
    """
    Calcule la clim mensuelle pour une catégorie spécifique.
    Catégories basées sur vmax (m/s) :
    TS: [26, 33[, Cat1: [33, 43[, Cat2: [43, 50[, Cat3: [50, 59[, Cat4: [59, 70[, Cat5: >= 70
    """
    # 1. On stocke la vmax maximale atteinte par chaque cyclone (année, numtc)
    tc_max_vmax = {} 
    # 2. On stocke les mois où chaque cyclone était actif
    tc_months = {} 
    ...
    with open(file_path, mode='r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if year_min <= dt.year <= year_max:
                ...
                # Filtre de base (Latitude + Critères ALADIN)
                if lat < 29:
                    if ALADIN and (vmax <= 26 or pmin >= 1005):
                        continue
                    
                    # On enregistre la vmax max pour cet ID
                    if tc_id not in tc_max_vmax or vmax > tc_max_vmax[tc_id]:
                        tc_max_vmax[tc_id] = vmax
                    
                    # On enregistre les mois d'activité
                    if tc_id not in tc_months:
                        tc_months[tc_id] = set()
                    tc_months[tc_id].add(dt.month)

    # 3. Définition des seuils de catégorie (en m/s)
    thresholds = {
        'TS': (26, 33),
        '1':  (33, 43),
        '2':  (43, 50),
    ...
    v_min, v_max = thresholds[target_cat]

    # 4. Comptage mensuel uniquement pour les TCs qui entrent dans la catégorie
    monthly_counts = {m: 0 for m in range(1, 13)}
    total_unique = 0

    for tc_id, max_v in tc_max_vmax.items():
        if v_min <= max_v < v_max:
            total_unique += 1
            for month in tc_months[tc_id]:
                monthly_counts[month] += 1

    num_years = year_max - year_min + 1
    clim_counts = [monthly_counts[m] / num_years for m in range(1, 13)]
    
    return clim_counts, total_unique