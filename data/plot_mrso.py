...
def get_mrsos_mean(path, is_jjaso, ymin, ymax):
...
# --- Calcul des données ---
print(f"Calcul des moyennes mrsos pour la période {YEAR_MIN}-{YEAR_MAX}...")
mean_ref = get_mrsos_mean(file_ref, JJASO, YEAR_MIN, YEAR_MAX)
mean_exp = get_mrsos_mean(file_exp, JJASO, YEAR_MIN, YEAR_MAX)
diff_mrsos = mean_exp - mean_ref
...
abs_min, abs_max = 0, 20
levels_abs = np.linspace(abs_min, abs_max, 21)
iso_levels_abs = np.arange(abs_min, abs_max + 1, 20) # Une ligne tous les 20 W/m²

# Paramètres pour la carte de différence
diff_limit = 3
levels_diff = np.linspace(-diff_limit, diff_limit, 13)
iso_levels_diff = np.arange(-diff_limit, diff_limit, 2) # Une ligne tous les 2 W/m²

titles = [f"NoRadDust - {YEAR_MIN}-{YEAR_MAX}", 
          f"Reférence - {YEAR_MIN}-{YEAR_MAX}", 
          "Différence (NoRadDust - Ref)"]

data_to_plot = [mean_exp, mean_ref, diff_mrsos] # Inversion pour coller aux titres
...
for i, ax in enumerate(axes):
    ...
    if i < 2:
        ...
        cbar_label = "Différence mrsos (W/m²)"
...
plt.suptitle(f"Humidité du sol en surface (mrsos) - {suffix}", 
