...
def get_rsds_mean(path, is_jjaso, ymin, ymax):
    """Charge les données rsds et calcule la moyenne temporelle."""
...
print(f"Calcul des moyennes RSDS pour la période {YEAR_MIN}-{YEAR_MAX}...")
...
iso_levels_diff = np.arange(-diff_limit, diff_limit, 5) # Une ligne tous les 2 W/m²

titles = [f"ALADIN-REF - {YEAR_MIN}-{YEAR_MAX}", 
          f"ALADIN-NoRadDust - {YEAR_MIN}-{YEAR_MAX}", 
          "Différence (NoRadDust-REF)"]

data_to_plot = [mean_ref, mean_exp, diff_rsds]
...
for i, ax in enumerate(axes):
    ...
    if i < 2:
        ...
        cbar_label = "Différence RSDS (W/m²)"
