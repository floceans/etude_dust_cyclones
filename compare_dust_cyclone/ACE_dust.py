...
aod_masked_2 = load_data(AOD_FILE_PATH_2, VAR_AOD_2, an_min, an_max)
...
years_aod = aod_annual_series_1.year.values
years_aod = aod_annual_series_2.year.values
...
ax1.set_title(f'Relation ACE vs AOD - {data_f} - ({an_min}-{an_max})', fontsize=14)
...
if data_f == 'aladin':
    ...
    ax2.plot([str(y) for y in years_aod], values_aod_2, color=color_aod, marker='s', linewidth=2, label=f'AOD ({data_f})')
