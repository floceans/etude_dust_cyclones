...
def get_mrso_climatology(path, name, is_jjaso, ymin, ymax, lat_min, lat_max, lon_min, lon_max):
    ...
    spatial_mean = ds_zone['mrso'].mean(dim=['y', 'x'])
...
clim_ref = get_mrso_climatology(file_ref, "REFERENCE", JJASO, YEAR_MIN, YEAR_MAX, LAT_MIN, LAT_MAX, LON_MIN, LON_MAX)
clim_exp = get_mrso_climatology(file_exp, "EXPERIENCE", JJASO, YEAR_MIN, YEAR_MAX, LAT_MIN, LAT_MAX, LON_MIN, LON_MAX)
...
plt.suptitle(f"MRSO clim mensu, Zone : {LAT_MIN}°N à {LAT_MAX}°N | {LON_MIN}°E à {LON_MAX}°E", 
